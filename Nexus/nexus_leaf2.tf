...
data "vsphere_network" "leaf2_net_eth1" {
  name          = "${vsphere_host_port_group.pod_portgroups["PG12"].name}"
  datacenter_id = data.vsphere_datacenter.datacenter.id
}

data "vsphere_network" "leaf2_net_eth2" {
  name                = "${vsphere_host_port_group.pod_portgroups["PG14"].name}"
  datacenter_id = data.vsphere_datacenter.datacenter.id
}

# (2X22) PODX_LEAF12_SRV12
data "vsphere_network" "leaf2_net_eth3" {
  name                = format("(2%s22) POD%s_LEAF12_SRV12", var.pod, var.pod)
  datacenter_id = data.vsphere_datacenter.datacenter.id
}

resource "vsphere_virtual_machine" "leaf2" {

  for_each = var.leaf2_data
  ...
  network_interface {
    ...
    use_static_mac = true
    mac_address  = format("02:00:fa:b%s:12:%02s", var.pod, each.value["mgmt"])
  }

  # Eth1/1
  network_interface {
    network_id   = data.vsphere_network.leaf2_net_eth1.id
    adapter_type = data.vsphere_virtual_machine.template.network_interface_types[0]
    use_static_mac = true
    mac_address  = format("02:00:fa:b%s:12:%02s", var.pod, each.value["eth1"])
  }

  # Eth1/2
  network_interface {
    network_id   = data.vsphere_network.leaf2_net_eth2.id
    adapter_type = data.vsphere_virtual_machine.template.network_interface_types[0]
    use_static_mac = true
    mac_address  = format("02:00:fa:b%s:12:%02s", var.pod, each.value["eth2"])
  }

  # Eth1/3
  network_interface {
    network_id   = data.vsphere_network.leaf2_net_eth3.id
    adapter_type = data.vsphere_virtual_machine.template.network_interface_types[0]
    use_static_mac = true
    mac_address  = format("02:00:fa:b%s:12:%02s", var.pod, each.value["eth3"])
  }

}

resource "null_resource" "leaf2_console" {
    for_each = var.leaf2_data

    triggers = {
      trigger = "${vsphere_virtual_machine.leaf2[each.key].uuid}"
    }
