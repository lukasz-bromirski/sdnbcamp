...
data "vsphere_network" "spine1_net_eth1" {
  name          = "${vsphere_host_port_group.pod_portgroups["PG11"].name}"
  datacenter_id = data.vsphere_datacenter.datacenter.id
}

data "vsphere_network" "spine1_net_eth2" {
  name                = "${vsphere_host_port_group.pod_portgroups["PG12"].name}"
  datacenter_id = data.vsphere_datacenter.datacenter.id
}


resource "vsphere_virtual_machine" "spine1" {

  for_each = var.spine1_data
  ...
  network_interface {
    ...
    use_static_mac = true
    mac_address  = format("02:00:fa:b%s:21:%02s", var.pod, each.value["mgmt"])
  }

  # Eth1/1
  network_interface {
    network_id   = data.vsphere_network.spine1_net_eth1.id
    adapter_type = data.vsphere_virtual_machine.template.network_interface_types[0]
    use_static_mac = true
    mac_address  = format("02:00:fa:b%s:21:%02s", var.pod, each.value["eth1"])
  }

  # Eth1/2
  network_interface {
    network_id   = data.vsphere_network.spine1_net_eth2.id
    adapter_type = data.vsphere_virtual_machine.template.network_interface_types[0]
    use_static_mac = true
    mac_address  = format("02:00:fa:b%s:21:%02s", var.pod, each.value["eth2"])
  }

}

resource "null_resource" "spine1_console" {
    for_each = var.spine1_data

    triggers = {
      trigger = "${vsphere_virtual_machine.spine1[each.key].uuid}"
    }
